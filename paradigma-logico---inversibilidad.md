...
En principio, todo predicado es inversible salvo que caiga en un [caso de no-inversibilidad](paradigma-logico---casos-de-no-inversibilidad.md).

Estos casos tienen que ver con las submetas de un predicado que requieren variables ligadas, en estas cosas es que hay que fijarse para saber qué hace falta [generar](paradigma-logico---generacion.md).
...
Inherentemente es más potente que una función, ya que te permite encontrar todos los valores para los cuales una relación se cumple (y esto tiene que ver también con la idea de [ múltiples resultados](paradigma-logico---multiples-respuestas.md)...).
