En el paradigma orientado a objetos existen diferentes formas de trabajar, los esquemas que usamos en la materia son el que se basa en [clases](clases.md) (tradicional) y el que se basa en prototipos (como se puede trabajar usando el Object Browser para Smalltalk, o nativamente por ejemplo en JavaScript).
...
¿Qué pasaría si estamos trabajando con el esquema de clases? Siempre que aparece comportamiento nuevo, necesitamos crear otra clase que lo incluya, con lo cual necesitaríamos crear una clase GolondrinaPerezosa que [herede](herencia.md) de Golondrina y definir allí el método \#descansar. De esta forma, pepona ya no debería ser una instancia de Golondrina sino de GolondrinaPerezosa.
